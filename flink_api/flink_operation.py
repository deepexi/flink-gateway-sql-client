...
from flink_api.flink_rest.response_models import FlinkJobDetail, Relation
...
class FlinkOperation:
    ...
    def _execute_statement_with_job_name(self, sql: str, job_name: str = None) -> None:
        ...
        if job_name:
            ...
            # need wait batch job finished:
            # assume modelA (with jobA produces data), and modelB (with jobB produces data)
            # modelB depends on modelA
            # if jobB starts before jobA finished, jobB will not get proper data of modelA
